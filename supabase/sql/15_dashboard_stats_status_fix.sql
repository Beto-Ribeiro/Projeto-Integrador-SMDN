...
-- Corrige indicadores do Dashboard para usar status real de Ocorrencia_Status.
...
as $$
  ...
  victims_from_tokens as (
    ...
    left join public."Perfis" p
      on p.prf_id = case
        when t.user_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' then t.user_id::uuid
        else null
      end
  ...
  victim_base as (
    select distinct on (victim_id) victim_id, lat, lng, victim_name, source, updated_at
    from victim_source
  ...
  select jsonb_build_object(
    'stats', jsonb_build_object(
      'activeOccurrences', (select count(*) from relato_base where status_key <> 'resolved'),
      'activeAlerts', (select count(*) from public."Alerta_Web" a where coalesce(a.alw_status, 'disparado') = 'disparado'),
      'criticalSeverity', (select count(*) from relato_base rb where rb.status_key <> 'resolved' and public.dashboard_severity_from_relato(rb.rel_descricao) = 'critical'),
      'resolvedToday', (select count(*) from relato_base rb where rb.status_key = 'resolved' and rb.ocs_updated_at::date = current_date),
    ...
    'recentOccurrences', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'city', case when lat is not null and lng is not null then 'Local informado' else 'Local não informado' end,
        'lat', lat,
        'lng', lng,
